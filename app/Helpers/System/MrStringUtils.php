...
class MrStringUtils
{
    public static function SanitizeFileName(string $filename, bool $transliterate = true): string
    {
        $r = trim($filename);
        ...
        $r = (string)preg_replace('/[^A-Za-z0-9\.\-\_]/i', '', $r);
    ...
    public static function SimpleDiff(string $old, string $new): array
    {
        $old = str_replace("\r\n", "\n", $old);
        $old = str_replace("\n\r", "\n", $old);
        $new = str_replace("\r\n", "\n", $new);
    ...
    public static function UnicodeCharsetCleanup(string $input): string
    {
        ...
        $r[] = ' ';//Ideographic Space
        $s[] = "\u{0300}";
        $r[] = ''; //Combining Grave Accent
        $s[] = "\u{0307}";
        $r[] = ''; //Combining Dot Above
