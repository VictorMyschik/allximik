...
{
    public const string PERMISSION_TRAVEL = 'Travel';
    public const string PERMISSION_SYSTEM = 'System';
    public const string PERMISSION_SETTINGS = 'Settings';
    public const string PERMISSION_LANGUAGE = 'Language';
    ...
    private function categoryMenu(): array
    {
        ...
        foreach (array_unique(array_column(Settings::getSettingList(), 'category')) as $item) {
            $list[$item] = Menu::make($item)
                ->icon('target')
                ->route('system.setup.list', ['category' => [$item]]);
        }
